...
from supervised_contrastive_loss import get_sup_contrastive_loss_fn
...
sup_cont_loss = get_sup_contrastive_loss_fn(apply_fts)

@jit
def ultimate_loss(params, envs):
    irm_loss, (losses, train_nll, train_acc, train_penalty) = loss_fn(params, envs)
    env_images = jnp.array([x['images'] for x in envs])
    env_labels = jnp.array([x['labels'] for x in envs])
    cont_loss = sup_cont_loss(params, env_images, env_labels)
    # t_loss = triplet_loss(params, triplet_images, None)
    ult_loss = irm_loss + cont_loss
